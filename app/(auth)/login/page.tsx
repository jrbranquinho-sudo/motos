...
export default function LoginPage() {
  ...
  const handleCredentialsSubmit = (e: React.FormEvent) => {
    ...
    const foundUser = users.find((u) => {
      ...
      // 1. Direct match on username or full email
      if (uUsername === clean || uEmail === clean) return true;

      // 2. Match by email prefix / username interchangeability
      if (cleanPrefix && (uUsername === cleanPrefix || uEmailPrefix === cleanPrefix)) return true;
    ...
    // Passwords accepted: user's configured password, default 'mot-os123', legacy fallbacks
    // Tolerant to mobile first-letter capitalization (e.g. Mot-os123 vs mot-os123)
            ...
            <form onSubmit={handleCredentialsSubmit} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  <input
                    type="text"
                    placeholder="jrbranquinho ou seu e-mail"
                    value={identifier}
                    onChange={(e) => {
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button
                    ...
                    className="text-[11px] text-zinc-400 hover:text-zinc-200 flex items-center gap-1 select-none"
                  >
                    {showLoginPassword ? (
                      <>
                ...
                <div className="relative">
                  <input
                    ...
                    autoCorrect="off"
                    spellCheck={false}
                    className="w-full bg-zinc-950 border border-zinc-700 rounded-xl pl-9 pr-10 py-2.5 text-sm text-zinc-100 focus:border-orange-500 focus:outline-none font-sans"
                    required
                  />
              ...
              {/* Quick Fill for Mobile Device Convenience */}
              <button
                type="button"
                onClick={() => {
                  setIdentifier("jrbranquinho");
                  setPassword("mot-os123");
                  setErrorMsg("");
                }}
                className="w-full py-1.5 px-3 rounded-lg bg-zinc-800/60 hover:bg-zinc-800 text-[11px] text-zinc-400 hover:text-orange-300 transition-colors flex items-center justify-center gap-1.5 border border-zinc-800 active:scale-98"
              >
                <KeyRound className="w-3.5 h-3.5 text-orange-400" />
                <span>Preencher Acesso Master (jrbranquinho)</span>
              </button>
